<template>
  ...
  <div v-if="hover" class="hint" :style="hintStyle">
    {{ hover.name }} ({{ hover.id }})
  </div>
</template>
...
onMounted(async () => {
  ...
  map.on("load", async () => {
    ...
    // Добавляем обработчик курсора после загрузки слоев
    map!.on("mousemove", (e: any) => {
      try {
        const features = map!.queryRenderedFeatures(e.point, {
          layers: ["sats", "sats-selected"],
        });
        ...
        if (features.length > 0) {
          ...
          if (canvas) {
            ...
            canvas.setAttribute("data-cursor", "default");
          }
        }
      } catch (error) {
        el.value!.style.cursor = "default";
  ...
  // Возвращаем курсор к default при выходе с карты
  map.on("mouseleave", () => {
    el.value!.style.cursor = "default";
  });
});
...
<style>
/* Принудительное изменение курсора для спутников */
.maplibregl-canvas-container .maplibregl-canvas {
  cursor: default !important;
}

/* Курсор pointer при наведении на спутники */
.maplibregl-canvas-container .maplibregl-canvas[data-cursor="pointer"] {
